...
import { OrgBranch, OrgCourseListItem } from '../models/orgadmin.models';
import { OrgAdminService } from '../orgadmin.service';

type StatusFilter = 'all' | 'published' | 'draft';

@Component({
  selector: 'app-orgadmin-courses',
  standalone: true,
  imports: [CommonModule, RouterLink],
  templateUrl: './orgadmin-courses.html',
})
export class OrgAdminCoursesPage {
  private readonly api = inject(OrgAdminService);
  private readonly searchInput$ = new Subject<string>();

  readonly loading = signal(true);
  readonly error = signal<string | null>(null);
  readonly courses = signal<OrgCourseListItem[]>([]);
