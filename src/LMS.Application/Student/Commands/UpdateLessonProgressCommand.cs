    ...
    : IRequestHandler<UpdateLessonProgressCommand, LessonDetailDto>
{
    ...
    public async Task<LessonDetailDto> Handle(UpdateLessonProgressCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            // Finishing every lesson marks the enrolment complete, but no longer
            // issues a certificate — that's now unlocked by passing the teacher's
            // exam(s). See CertificateIssuanceService, invoked from the grading flow.
